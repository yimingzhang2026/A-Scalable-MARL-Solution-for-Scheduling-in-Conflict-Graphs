...
class EnvRunner(Runner):
    ...
    def warmup(self):
        ...
        share_obs = []
        for o in obs:
            share_obs.append( [float(any(col)) for col in zip(*o)])
        share_obs = np.array(share_obs) # shape = [env_num, agent_num]
    ...
    def eval_100k(self):
        ...
        print('restore model from {}'.format(self.model_dir))
        ...
        for trial in range(trials):
            print("trial is {}".format(trial))
            eval_finish = False
            eval_actions = [[] for i in range(self.num_agents)]
            eval_obs = self.eval_envs.reset()
            ...
            for eval_step in range(self.episode_length):
                ...
                # Obser reward and next obs
                eval_obs, eval_rewards, eval_dones, eval_infos = self.eval_envs.step_eval(eval_actions_env)
                #print(eval_obs)
                eval_rnn_states[eval_dones == True] = np.zeros(
                    ((eval_dones == True).sum(), self.recurrent_N, self.hidden_size),
                    dtype=np.float32,
                ...
                eval_finish = (any(eval_dones[0,:]) == True)
                if eval_finish:
                    info = self.eval_envs.envs[0].env.get_info_ep()
                    print(info)
        ...
        data = ep_infos  # Get the data dictionary
        np_path = str(self.run_dir) + 'nn_ch1_20nodes_dr0.15.npz'
        save_data(np_path, data)             
